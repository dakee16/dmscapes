...
export default function PlannerStudio({canvas,get2DPng,shopping,products,total,budget,subtitle,history,onReset,extras,unplaced}:{canvas:ReactNode;get2DPng:()=>string|null;shopping:ReactNode;products:Product[];total:number;budget:number;subtitle:string;history:{canUndo:boolean;canRedo:boolean;undo:()=>void;redo:()=>void};onReset:()=>void;extras?:ReactNode;unplaced?:ReactNode}){
  ...
  function open(next:Panel){setPanel(next);setMobileOpen(true);if(next!=="item")setMoveMode(false);}
  ...
  return <div ref={root} className={s.studio+" "+(expanded?s.expanded:"")} data-testid="planner-studio"
    ...
    <header className={s.header}>
      ...
      <div className={s.viewSwitch} role="group" aria-label="Planner view"><button aria-pressed={view==="2d"} onClick={()=>{setView("2d");setMoveMode(false);}}>2D plan</button><button aria-pressed={view==="3d"} onClick={()=>setView("3d")}>3D room</button></div>
    ...
    <div className={s.workspace}>
      <nav className={s.rail} aria-label="Studio tools">
        {([["furnish","▦","Furnish"],["style","◐","Style"],["room","⌑","Room"],["shop","⊞","Shop"]] as const).map(([key,icon,label])=><button key={key} aria-pressed={panel===key} onClick={()=>open(key)}><span aria-hidden="true">{icon}</span>{label}</button>)}
        <button aria-pressed={panel==="checks"} onClick={()=>open("checks")}><span aria-hidden="true">✓</span>Checks{issues.length>0&&<b>{new Set(issues.map(i=>i.id)).size}</b>}</button>
        <button onClick={()=>open("help")} aria-label="Studio guide"><span aria-hidden="true">?</span>Help</button>
      </nav>
      <section className={s.viewport} aria-label="Room workspace">
        ...
        <div className={s.renderArea}>
          <div className={s.sceneLayer} style={{visibility:view==="3d"?"visible":"hidden",pointerEvents:view==="3d"?"auto":"none"}} aria-hidden={view!=="3d"}>
            <RoomScene ref={scene} room={room} items={items} hidden={hidden} excluded={excluded} locked={locked} selectedId={selectedId} style={style} products={products} snap={snap} walls={walls} moveMode={moveMode}
        ...
        {view==="3d"&&<>
          <div className={s.cameraBar} role="group" aria-label="Camera controls">
            ...
            <button aria-label="Reset camera" onClick={()=>preset("room")}>Reset view</button>
          </div>
          <p className={s.gestureHint}>{moveMode?"Move mode: drag the selected furniture. Tap Stop moving when done.":"Drag empty space to look around. Select a piece to arrange it."}</p>
        </>}
        <div className={s.editBar}>
          ...
          {view==="3d"&&<><label><input type="checkbox" checked={snap} onChange={e=>setSnap(e.target.checked)}/>Snap</label><label className={s.wallSelect}>Walls<select aria-label="Wall visibility" value={walls} onChange={e=>setWalls(e.target.value)}><option value="auto">Auto</option><option value="all">All</option><option value="hidden">Hidden</option></select></label></>}
          <button onClick={()=>setResetConfirm(v=>!v)}>Reset layout</button>
        </div>
      ...
      <aside className={s.panel+" "+(mobileOpen?s.mobileOpen:"")} aria-label={titles[panel]}>
        ...
        <div className={s.panelContent}>
          {panel==="furnish"&&<><p className={s.eyebrow}>Your room inventory</p><h2>Make space.</h2><p className={s.muted}>Choose a piece to move, measure, or restyle it. Fixed and hidden items stay listed here.</p>
          ...
          {panel==="shop"&&shopping}
          {panel==="checks"&&<><p className={s.eyebrow}>A second look</p><h2>Placement checks</h2><p className={s.muted}>Checks flag overlap, wall boundaries, ceiling height, and proximity to inward door swings. They are a guide, not a guarantee of fit.</p>
